...
util.PrecacheSound("guy1/dunkey.mp3")
util.PrecacheSound("guy1/dontstopcomin.mp3")
util.PrecacheSound("guy1/dunkey2.mp3")
util.PrecacheSound("guy1/bothhavelayers2.mp3")
util.PrecacheSound("guy1/bothhavelayers.mp3")
util.PrecacheSound("guy1/ogreshavelayers.mp3")
util.PrecacheSound("guy1/onionshavelayers.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_guy1_acquire_distance = CreateConVar("npc_guy1_acquire_distance", 2500, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                "The maximum distance at which guy1 will chase a target.")

local npc_guy1_spawn_protect = CreateConVar("npc_guy1_spawn_protect", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                             "If set to 1, guy1 will not target players or hide within 200 units of a spawn point.")

local npc_guy1_attack_distance = CreateConVar("npc_guy1_attack_distance", 80, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The reach of guy1's attack.")

local npc_guy1_attack_interval = CreateConVar("npc_guy1_attack_interval", 0.2, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                               "The delay between guy1's attacks.")

local npc_guy1_attack_force = CreateConVar("npc_guy1_attack_force", 800, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                            "The physical force of guy1's attack. Higher values throw things farther.")

local npc_guy1_smash_props = CreateConVar("npc_guy1_smash_props", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                           "If set to 1, guy1 will punch through any props placed in their way.")

local npc_guy1_hiding_scan_interval = CreateConVar("npc_guy1_hiding_scan_interval", 3, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                    "guy1 will only seek out hiding places every X seconds. This can be an expensive operation, so it is not " ..
                                                    "recommended to lower this too much. However, if distant guy1s are not hiding from you quickly enough, " ..
                                                    "you may consider lowering this a small amount.")

local npc_guy1_hiding_repath_interval = CreateConVar("npc_guy1_hiding_repath_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                      "The path to guy1's hiding spot will be redetermined every X seconds.")

local npc_guy1_chase_repath_interval = CreateConVar("npc_guy1_chase_repath_interval", 0.1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                     "The path to and position of guy1's target will be redetermined every X seconds.")

local npc_guy1_expensive_scan_interval = CreateConVar("npc_guy1_expensive_scan_interval", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE),
                                                       "Slightly expensive operations (distance calculations and entity searching) will occur every X seconds.")

local npc_guy1_force_download = CreateConVar("npc_guy1_force_download", 1, bit.bor(FCVAR_GAMEDLL, FCVAR_DEMO, FCVAR_SERVER_CAN_EXECUTE, FCVAR_ARCHIVE),
                                              "If set to 1, clients will be forced to download guy1 resources (restart required after changing).\n" ..
                                              "WARNING: If this is not set to 1, clients will not be able to see or hear guy1!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_LOCKOUT_TIME = 5

if (npc_guy1_force_download:GetBool()) then
	resource.AddWorkshop("2849870288")
end

util.AddNetworkString("guy1_nag")
util.AddNetworkString("guy1_nagresponse")
util.AddNetworkString("guy1_navgen")
...
local VECTOR_guy1_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_guy1_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_guy1: found %d suitable (%d unsuitable) hiding places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
end
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other guy1s, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "guy1MissingNavmeshNag", function(ply, ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_guy1") then return end
	if (navmesh.GetNavAreaCount() > 0) then return end
	if (ply.guy1_HasBeenNagged) then return end
	ply.guy1_HasBeenNagged = true

	-- Try to explain why guy1 isn't working.
	net.Start("guy1_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if (not navmesh.IsGenerating()) then
		print("npc_guy1: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_guy1: Navmesh generation aborted after " .. timeElapsedStr)
	end
end

net.Receive("guy1_nagresponse", function(len, ply)
	if (net.ReadBit() == 0) then
		ply.guy1_HasBeenNagged = false
		return
	end
	...
	for _, guy1 in pairs(ents.FindByClass("npc_guy1")) do
		guy1:Remove()
	end

	print("npc_guy1: Beginning nav_generate requested by " .. ply:Name())
	...
	if (navmesh.IsGenerating()) then
		...
		hook.Add("ShutDown", "guy1NavGen", navEndGenerate)
	else
		print("npc_guy1: nav_generate failed to initialize")
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_guy1_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- Spawn protection! Ignore players within 200 units of a spawn point if `npc_guy1_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if (npc_guy1_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200)) then
			continue
		end
...
function ENT:AttackNearbyTargets(radius)
	...
	for _, ent in pairs(nearEntities) do
		if (isValidTarget(ent)) then
			...
			if (ent:IsPlayer() and IsValid(ent:GetVehicle())) then
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_guy1_attack_force:GetInt() * phys:GetMass()), hitOffset)
				end
			...
			-- Give the player a good whack. guy1 means business.
			-- This is for those with god mode enabled.
			ent:SetVelocity(hitDirection * npc_guy1_attack_force:GetInt() + vector_up * 500)
			...
			dmgInfo:SetDamageForce((hitDirection * npc_guy1_attack_force:GetInt() + vector_up * 500) * 100)
		...
		elseif (ent:GetMoveType() == MOVETYPE_VPHYSICS) then
			if (not npc_guy1_smash_props:GetBool()) then continue end
			...
			for id = 0, ent:GetPhysicsObjectCount() - 1 do
				...
				if (IsValid(phys)) then
					...
					phys:ApplyForceOffset(hitDirection * (npc_guy1_attack_force:GetInt() * mass), hitOffset)
				end
			end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_guy1_attack_distance:GetInt()
	if (xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance) then
		...
		self:EmitSound((jumpHeight > 500 and "guy1/dunkey2.mp3" or "guy1/dunkey.mp3"), 350, 100)
	end
end
...
local tauntSounds = {
	"guy1/bothhavelayers2.mp3",
	"guy1/ogreshavelayers.mp3",
	"guy1/bothhavelayers.mp3",
	"guy1/onionshavelayers.mp3"
}
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	if (currentTime - self.LastTargetSearch > npc_guy1_expensive_scan_interval:GetFloat()) then
	...
	if (IsValid(self.CurrentTarget)) then
		...
		if (currentTime - self.LastAttack > npc_guy1_attack_interval:GetFloat()) then
			if (self:AttackNearbyTargets(npc_guy1_attack_distance:GetInt())) then
		...
		if (currentTime - self.LastPathRecompute > npc_guy1_chase_repath_interval:GetFloat()) then
		...
		if (self:IsOnGround()) then
			if (currentTime - self.LastJumpScan >= npc_guy1_expensive_scan_interval:GetFloat()) then
				self:AttemptJumpAtTarget()
				self.LastJumpScan = currentTime
			end
		end
	else
		if (currentTime - self.LastHidingPlaceScan >= npc_guy1_hiding_scan_interval:GetFloat()) then
		...
		if (self.HidingSpot ~= nil) then
			if (currentTime - self.LastPathRecompute >= npc_guy1_hiding_repath_interval:GetFloat()) then
				self.LastPathRecompute = currentTime
	...
	if (currentTime - self.LastCeilingUnstick >= npc_guy1_expensive_scan_interval:GetFloat()) then
...
--[[local lastFrame = 0
local guy1sLastFrame = 0
hook.Add("Tick", "guy1Benchmark", function()
	local now = SysTime()
	local numguy1s = #ents.FindByClass("npc_guy1")

	if (timeAll ~= 0) then

		local guy1Time = timeAll * 1000
		...
		print(string.format("RunBehaviour() avg %.2fms total %.2fms (%.2fms frame, ratio %.2f%%) (%d exist, %d thinking (%.2f%%))",
		                    guy1Time / guy1sLastFrame, guy1Time, frameTime, (guy1Time / frameTime) * 100, guy1sLastFrame, numThink, (numThink / guy1sLastFrame) * 100))
		timeAll = 0
		numThink = 0
	end

	lastFrame = now
	guy1sLastFrame = numguy1s
end)]]
...
killicon.Add("npc_guy1", "guy1/npc_guy1_killicon", color_white)
language.Add("npc_guy1", "guy1")
...
local lastPanic = 0 -- The last time we were in music range of a guy1.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_guy1_music_volume = CreateConVar("npc_guy1_music_volume", 1, bit.bor(FCVAR_CLIENTDLL, FCVAR_DEMO, FCVAR_ARCHIVE),
                                            "Maximum music volume when being chased by guy1. (0-1, where 0 is muted)")

local MUSIC_RESTART_DELAY = 2 -- If another guy1 comes in range before this delay is up, the music will continue where it left off.

local MUSIC_CUTOFF_DISTANCE   = 1000 -- Beyond this distance, guy1s do not count to music volume.
local MUSIC_PANIC_DISTANCE    = 200 -- Max volume is achieved when MUSIC_guy1_PANIC_COUNT guy1s are this close, or an equivalent score.
local MUSIC_guy1_PANIC_COUNT = 8 -- That's a lot of guy1.

local MUSIC_guy1_MAX_DISTANCE_SCORE = (MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_guy1_PANIC_COUNT

local function updatePanicMusic()
	if (#ents.FindByClass("npc_guy1") == 0) then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("guy1PanicMusicUpdate")
	...
	if (panicMusic == nil) then
		if (IsValid(LocalPlayer())) then
			panicMusic = CreateSound(LocalPlayer(), "guy1/dontstopcomin.mp3")
			panicMusic:Stop()
		else
	...
	if (npc_guy1_music_volume:GetFloat() <= 0 or not IsValid(LocalPlayer())) then
		panicMusic:Stop()
		return
	end
	...
	for _, ent in pairs(nearEntities) do
		if (IsValid(ent) and ent:GetClass() == "npc_guy1") then
	...
	local musicVolume = math.min(1, totalDistanceScore / MUSIC_guy1_MAX_DISTANCE_SCORE)
	...
	if (musicVolume > 0) then
		...
		if (not LocalPlayer():Alive()) then
			-- Quiet down so we can hear guy1 taunt us.
	...
	musicVolume = math.max(0.01, musicVolume * math.Clamp(npc_guy1_music_volume:GetFloat(), 0, 1))
...
local function startTimer()
	if (not timer.Exists("guy1PanicMusicUpdate")) then
		timer.Create("guy1PanicMusicUpdate", 0.05, 0, updatePanicMusic)
		DevPrint(4, "Beginning music timer.")
	end
end

local guy1Material = Material("guy1/npc_guy1.png", "smooth mips")
local drawOffset = Vector(0, 0, 64)
function ENT:RenderOverride()
	render.SetMaterial(guy1Material)
...
hook.Add("OnEntityCreated", "guy1Initialize", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_guy1") then return end

	local guy1EntTable = scripted_ents.GetStored("npc_guy1")

	table.Merge(ent, guy1EntTable.t) --HACK!!! Because this isn't done for us.
	ent:CallOnRemove("guy1_removed", guy1Deregister)
end)

hook.Add("NetworkEntityCreated", "guy1NetInit", function(ent)
	if (not IsValid(ent)) then return end
	if (ent:GetClass() ~= "npc_guy1") then return end
...
surface.CreateFont("guy1HUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("guy1HUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("guy1 is studying this map.", "guy1HUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "guy1HUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)

	local elapsed = (SysTime() - generateStart)
	local elapsedStr = string_ToHMS(elapsed)
	draw.SimpleTextOutlined("Time Elapsed:", "guy1HUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "guy1HUDSmall", ScrW() / 2, ScrH() * 3/4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	...
	draw.SimpleTextOutlined(flavourText, "guy1HUDSmall", ScrW() / 2, ScrH() * 4/5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("guy1_navgen", function()
	if (net.ReadBit() == 1) then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "guy1NavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message([[Oh no. guy1 doesn't even know where to start with this map.
...
local function navWarning()
	Derma_Query([[It will take a while for guy1 to figure this map out.
...
-- Lazy
net.Receive("guy1_nag", function()
	if (game.SinglePlayer()) then
		Derma_Query([[Uh oh! guy1 doesn't know this map.
Would you like guy1 to learn it?]],
		            "This map is currently not guy1-compatible!",
		            "Yes", navWarning,
		            "No", nagAgain,
		            "No. Do not ask again.")
	else
		Derma_Query([[Uh oh! guy1 doesn't know this map. He won't be able to move!
Because you're not in a single-player game, he isn't able to learn it.

Ask the server host about teaching this map to guy1.]],
		            "This map is currently not guy1-compatible!",
		            "Alrighty then.", nagAgain,
		            "Alright. I don't care, don't say this again.")
	end
end)
